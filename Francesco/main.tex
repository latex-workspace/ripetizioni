...
\begin{center}
	\verb|ArrayList<Integer> lista = new ArrayList<>(Arrays.asList(1,2,3));|
\end{center}
...
\begin{esercizio}{Operazioni arraylist}
	Scrivere un programma che chieda all'utente un numero \verb|n| di interi da terminale. L'applicazione deve innanzitutto chiedere \verb|n| all'utente e successivamente leggere \verb|n| interi in un arraylist. Una volta letti tutti i numeri viene stampato un prompt che chiede quale operazione eseguire. Le opzioni sono le seguenti:
	\begin{enumerate}
		\item \textit{Verifica numero}: viene chiesto un numero e l'applicazione deve verificare se il numero è presente nell'arraylist
		\item \textit{Rimuovi numero}: viene chiesto un numero e l'applicazione deve rimuovere \textit{tutte} le occorrenze di quel numero nell'arraylist
		\item \textit{Sort}: chiede se vuole ordinare l'arraylist in ordine crescente o decrescente e stampa il risultato ordinato. Usare un algoritmo di sort scritto a mano, non quello implementato nelle classi standard!
		\item \textit{Check sort}: stampa \verb|true| se il vettore è ordinato, \verb|false| altrimenti
		\item \textit{Reverse}: inverte l'ordine del vettore e lo stampa
		\item \textit{Quit}: esce dall'applicazione
	\end{enumerate}
\end{esercizio}
