...
Intuitivamente, il lavoro è il concetto fisico che descrive "l'impatto" di una forza su di un sistema. In particolare, lega la forza allo spostamento che questa genera.
\begin{definizione}{Lavoro}
	Il lavoro $ L $ è dato dalla forza $ F $ moltiplicata per lo spostamento $ s $ che questa forza genera, e per il coseno dell'angolo $ \theta $ che la forza forma con lo spostamento:
	\[
		L = F \cdot s \cdot \cos(\theta)
	\]
\end{definizione}
Il coseno dell'angolo è importante in quanto solo la componente della forza che è parallela allo spostamento contribuisce al lavoro. Se la forza è perpendicolare allo spostamento, il lavoro è zero, poiché $ \cos(90^\circ) = 0 $.
\section{Energia}
In fisica esistono diversi modi per classificare l'energia, ma in generale possiamo dire che l'energia è la capacità di un sistema di compiere lavoro. Esistono diverse forme di energia:
\begin{itemize}
	\item Energia meccanica:
	      \begin{itemize}
		      \item Energia cinetica: energia data dal moto e dalla massa del sistema
		      \item Energia potenziale: energia data dalla posizione o stato del sistema
	      \end{itemize}
	\item Energia termica: energia data dalla temperatura del sistema
	\item Energia elettrica: energia data dalla presenza di cariche elettriche e dai campi elettrici
	\item Energia chimica: energia data dalle reazioni chimiche e dalle interazioni tra le molecole
	\item Energia nucleare: energia data dalle interazioni tra i nuclei atomici
	\item Energia radiante: energia data dalle onde elettromagnetiche, come la luce
	\item Energia sonora: energia data dalle onde sonore e dalle vibrazioni
\end{itemize}
Noi, per fortuna, ci concentreremo solo sull'energia meccanica, in particolare su quella cinetica e potenziale. Ciò che importa, tuttavia, è che l'energia non si crea ne si distrugge, ma passa da una forma all'altra.
\begin{teorema}{Conservazione dell'energia}
	In un sistema isolato, l'energia totale rimane costante nel tempo. Ciò significa che l'energia può essere trasformata da una forma all'altra, ma la quantità totale di energia nel sistema non cambia.
\end{teorema}
\subsubsection{Energia cinetica}
Come già anticipato, l'energia cinetica è l'energia associata al movimento di un oggetto. Dipende dalla massa dell'oggetto e dalla sua velocità. La formula per calcolare l'energia cinetica è la seguente:
\begin{definizione}{Energia cinetica}
	L'energia cinetica $ K $ di un oggetto è data dalla formula:
	\[
		K = \frac{1}{2} m v^2
	\]
	dove $ m $ è la massa dell'oggetto e $ v $ è la sua velocità.
\end{definizione}

\subsubsection{Energia potenziale gravitazionale}
Un corpo che si trova ad una data altezza rispetto a un punto di riferimento (di solito il suolo) possiede energia potenziale gravitazionale. Questa energia dipende dalla massa del corpo, dall'altezza a cui si trova e dall'accelerazione dovuta alla gravità. La formula per calcolare l'energia potenziale gravitazionale è la seguente:
\begin{definizione}{Energia potenziale gravitazionale}
	L'energia potenziale gravitazionale $ U $ di un oggetto è data dalla formula:
	\[
		U = m g h
	\]
	dove $ m $ è la massa dell'oggetto, $ g $ è l'accelerazione dovuta alla gravità (circa $ 9.81 \operatorname{m/s^2} $ sulla Terra) e $ h $ è l'altezza dell'oggetto rispetto al punto di riferimento.

\end{definizione}

\subsubsection{Energia potenziale elastica}
Un corpo che è stato deformato (ad esempio, compresso o allungato) possiede energia potenziale elastica. Questa energia dipende dalla forza applicata per deformare il corpo e dalla quantità di deformazione. La formula per calcolare l'energia potenziale elastica è la seguente:
\begin{definizione}{Energia potenziale elastica}
	L'energia potenziale elastica $ U $ di un oggetto è data dalla formula:
	\[
		U = \frac{1}{2} k x^2
	\]
	dove $ k $ è la costante elastica del materiale e $ x $ è la quantità di deformazione (ad esempio, l'allungamento o la compressione) dell'oggetto.
\end{definizione}
\subsubsection{Conservazione dell'energia meccanica}
In un sistema isolato, l'energia meccanica totale (la somma dell'energia cinetica e dell'energia potenziale) rimane costante nel tempo. Ciò significa che l'energia può essere trasformata da una forma all'altra, ma la quantità totale di energia meccanica nel sistema non cambia.
\begin{teorema}{Conservazione dell'energia meccanica}
	In un sistema isolato, l'energia meccanica totale rimane costante nel tempo
\end{teorema}

\subsection{Lavoro ed energia}
Ora che abbiamo visto l'energia, possiamo iniziare a parlare di lavoro come \textit{meccanismo secondo il quale l'energia può entrare o uscire da un sistema}. Visto il legame intimo fra energia e lavoro possiamo anche esprimerlo con formule matematiche. In particolare, il lavoro complessivo delle forze che agiscono su di un sistema è dato dalla variazione dell'energia cinetica. In formule:
\begin{teorema}{Energia cinetica e lavoro}
	Il lavoro totale $ L $ compiuto dalle forze che agiscono su un sistema è uguale alla variazione dell'energia cinetica $ \Delta E_c $ del sistema:
	\[
		L = \Delta E_c
	\]
\end{teorema}
...
\begin{align*}
	L & = F \cdot s \\
\end{align*}
per la seconda legge di Newton, $ F = m \cdot a $, quindi
\[
	L = m \cdot a \cdot s \\
\]
Dopodichè vogliamo rimuovere $ s $ dalla formula appena ottenuta, in quanto non partecipa direttamente nella formula dell'energia cinetica. Dunque ragioniamo così:
\[
	\begin{cases}
		v = v_0 + at \\
...
\[
	t = \frac{v - v_0}{a}
\]
e inserendo nella seconda ottengo:
\begin{align*}
	s & = \frac{1}{2} a \left(\frac{v - v_0}{a}\right)^2 + v_0 \cdot \frac{v - v_0}{a}           \\
	  & = \frac{1}{2} \cdot \frac{\left(v - v_0\right)^2}{a} + \frac{v_0\left(v - v_0\right)}{a}
\end{align*}
raccolgendo $ \frac{v - v_0}{a} $ e ottengo
\begin{align*}
	s & = \left(\frac{v - v_0}{a}\right) \cdot  \left(\frac{v - v_0}{2} + v_0\right) \\
	  & = \left(\frac{v - v_0}{a}\right) \cdot  \left(\frac{v + v_0}{2}\right)       \\
	  & = \frac{\left(v - v_0\right) \cdot \left(v + v_0\right)}{2a}
\end{align*}
quest'ultima, per il prodotto notevole di somma per differenza diventa:
\[
	s = \frac{v^2  - v_0^2 }{2a}
\]
inserendo questa nella prima formula ottengo:
\[
	L = m \cdot a \cdot \frac{v^2  - v_0^2 }{2a} = \frac{1}{2} m v^2  - \frac{1}{2} m v_0^2  = \Delta E_c
\]

\subsubsection{Potenza e watt}
Spesso è utile capire quanta potenza un apparecchio richiede per operare. Questo concetto è piuttosto simile a quello della velocità, in quanto indica la quantità di lavoro che un apparecchio è in grado di compiere in un certo intervallo di tempo. La potenza è data da:
\begin{definizione}{Potenza}
	La potenza $ P $ è data dal lavoro $ L $ compiuto da un sistema diviso per il tempo $ \Delta t $ impiegato a compiere quel lavoro:
	\[
		P = \frac{L}{\Delta t}
	\]
	l'unità di misura della potenza è il watt $ \left[w\right] $, che è definito come un joule al secondo:
\end{definizione}
\section{Quantità di moto e impulso}
\begin{definizione}{Quantità di moto}
	La quantità di moto $ p $ di un oggetto è data dalla sua massa $ m $ moltiplicata per la sua velocità $ v $:
	\[
		\vec{p}  = m \cdot \vec{v}
	\]
\end{definizione}
Si noti come la quantità di moto è una quantità vettoriale, in quanto dipende dalla direzione del movimento dell'oggetto. La quantità di moto è importante perché è una grandezza conservata in un sistema isolato, il che significa che la quantità totale di moto in un sistema isolato rimane costante nel tempo, ma questo lo affronteremo più approfonditamente più avanti
\begin{definizione}{Impulso}
	L'impulso $ J $ è dato dalla forza media $ \vec{F_{\text{med}}} $ moltiplicata per il tempo $ \Delta t $ durante il quale la forza agisce su un oggetto:
	\[
		\vec{J}=\vec{F_{\text{med}}} \cdot \Delta t
	\]
	NB: si parla di impulso solo quando la forza agisce per un breve intervallo di tempo, ad esempio nel caso degli urti
\end{definizione}
C'è un legame molto stretto fra quantità di moto e impulso, in quanto l'impulso subito da un oggetto è uguale alla variazione della sua quantità di moto:
\begin{teorema}{Legame tra quantità di moto e impulso}
	L'impulso $ J $ subito da un oggetto è uguale alla variazione della sua quantità di moto $ \Delta p $:
	\[
		\vec{J} = \Delta \vec{p}
	\]
\end{teorema}

In più, la quantità di moto totale in un sistema isolato è conservata, il che significa che la quantità totale di moto prima di un evento (ad esempio, un urto) è uguale alla quantità totale di moto dopo l'evento:
\begin{teorema}{Conservazione quantità di moto}
	In un sistema isolato (in cui agiscono solo forze interne), la quantità di moto totale rimane costante nel tempod:
	\[
...
\subsection{Urti}
\begin{definizione}{Urto elastico}
	Un urto è detto \textit{elastico} se la quantità di moto totale e l'energia cinetica totale del sistema sono entrambe conservate durante l'urto.
	\begin{align*}
		\sum \vec{p}_{\text{i}} & = \sum \vec{p}_{\text{f}} &  & K_{\text{i}} = K_{\text{f}}
	\end{align*}
\end{definizione}
Quindi un urto elastico è un urto che non comprta perdita di energia. Contrariamente, se vi è pertita di energia, allora l'urto è detto \textit{anelastico}
\begin{definizione}{Urto anelastico}
	Un urto è detto \textit{anelastico} se la quantità di moto totale si conserva, ma l'energia cinetica totale \textit{no}.
	\begin{align*}
		\sum \vec{p}_{\text{i}} & = \sum \vec{p}_{\text{f}} &  & K_{\text{i}} = aK_{\text{f}} \quad a \in\left[0,1\right)
	\end{align*}
\end{definizione}
\begin{definizione}{Urto completamente anelastico}
	Un urto completamente anelastico è un urto in cui i corpi coinvolti si uniscono dopo l'urto, formando un unico corpo che si muove con una velocità comune. In questo tipo di urto, la quantità di moto totale si conserva, ma l'energia cinetica totale non si conserva.
	\[
...
\section{Coniche}
Le coniche sono curve che possono essere ottenute intersecando un piano con un cono. Esistono diverse tipologie di coniche, tra cui ellissi, parabole e iperboli. Ognuna di queste curve ha proprietà uniche e può essere descritta da un'equazione specifica. In questa sezione, esploreremo le caratteristiche di ciascuna di queste coniche e come rappresentarle graficamente.

\subsection{Ellisse}
Iniziamo mostrando un esempio di ellisse. Intuitivamente non è altro che una circonferenza "schiacciaca"
\begin{figure}[H]
	\begin{tikzpicture}
		\begin{axis}[
				xmin=-6, xmax=6,
				ymin=-4,ymax=4,
				restrict y to domain = -4:4, domain=-5:5, width=0.98\textwidth, height=0.5\textwidth, grid=major, samples=200,  ylabel=$f(x)$, xlabel=$x$]
			\addplot[black, thick] ({5*cos(deg(x))},{3*sin(deg(x))});
			\draw (-5,0)--(5,0);
			\draw (0,-3)--(0,3);

			\draw [thick, mutedred](0,0)--(5,0) node [midway, above] {$a$};
			\draw [thick, mutedblue](0,0)--(0,3) node [midway, right] {$b$};
			\draw [thick, mutedgreen](0,0)--(-4, 0) node [midway, above] {$c$};

			\node [whitedot, label=$f_1$] at (4,0){};
			\node [whitedot, label=$f_2$] at (-4,0){};
			\node [whitedot, label=45:{$C$}] at (0,0){};

			\node [whitedot, label=180:$ v_1 $] at (-5,0){};
			\node [whitedot, label=-90:$ v_2 $] at (0, -3){};
			\node [whitedot, label=90:$ v_3 $] at (5,0){};
			\node [whitedot, label=$ v_4 $] at (0, 3){};

			% \addplot [red,thick,domain=-2:2] ({cosh(x)}, {sinh(x)});
			% \addplot [red,thick,domain=-2:2] ({-cosh(x)}, {sinh(x)});
		\end{axis}
	\end{tikzpicture}
	\caption{Esempio di ellisse}
	\label{fig:ellisse}
\end{figure}

\begin{definizione}{Ellisse}
	L'ellisse è il luogo dei punti del piano per i quali la somma delle distanze da due punti fissi, detti fuochi, è costante. In particolare, se $ P $ è un punto qualsiasi dell'ellisse e $ f_1 $ e $ f_2 $ sono i due fuochi, allora:
	\[
		\overline{f_1P}+\overline{f_2P} = \text{costante}
	\]
	in particolare, se $ a $ è la distanza tra il centro dell'ellisse e uno dei vertici, e $ c $ è la distanza tra il centro dell'ellisse e uno dei fuochi, allora vale la relazione:
	\[
		\overline{f_1P}+\overline{f_2P} = 2a
	\]
\end{definizione}
Partendo da questa definizione possiamo ricavarne la formula, la quale è molto simile a quella della circonferenza:
\[
	\frac{\left(x - x_c\right)^2 }{a^2} + \frac{\left(y - y_c\right)^2 }{b^2} = 1
\]
in particolare si noti che se $ a = b $, ossia se i due assi sono uguali, allora l'ellisse diventa una circonferenza.

\subsubsection{Vertici}
I vertici sono i punti indicati in \cref{fig:ellisse} con $ v_1 $, $ v_2 $, $ v_3 $ e $ v_4 $. Essi sono i punti di intersezione fra gli assi e l'ellise stessa. Date le coordinate del centro $ c = \left(x_c, y_c\right) $, allora è immediato vedere che questi hanno coordinate:
\begin{align*}
	v_1  = \left(x_c - a, y_c\right) &  & v_2  = \left(x_c, y_c - b\right) \\
	v_3  = \left(x_c + a, y_c\right) &  & v_4  = \left(x_c, y_c + b\right)
\end{align*}

\subsubsection{Fuochi}
I fuochi sono i punti indicati in \cref{fig:ellisse} con $ f_1 $ e $ f_2 $. Essi sono i punti che definiscono l'ellisse, in quanto la somma delle distanze da questi punti è costante. Questi punti sono simmetrici rispetto al centro dell'ellisse e sono posizionati sull'asse orizzontale se $ a > b $, su quello verticale altrimenti:
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
					xmin=-6, xmax=6,
					ymin=-6,ymax=6,
					restrict y to domain = -6:6, domain=-6:6, width=0.98\linewidth, height=0.98\linewidth, grid=major, samples=200,  ylabel=$f(x)$, xlabel=$x$]
				\addplot[black, thick] ({5*cos(deg(x))},{3*sin(deg(x))});
				\draw (-5,0)--(5,0);
				\draw (0,-3)--(0,3);
			...
			\end{axis}
		\end{tikzpicture}
		\subcaption{Ellisse orizzontale}
		\label{fig:ellisse orizzontale}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.45\textwidth}
		\centering
		\begin{tikzpicture}
			\begin{axis}[
					...
					height=0.98\linewidth,
					grid=major,
					samples=200,
					ylabel=$f(x)$,
					xlabel=$x$
				]
				\addplot[black, thick] ({3*cos(deg(x))},{5*sin(deg(x))});
				\draw (-3,0)--(3,0);
				\draw (0,-5)--(0,5);

				\draw [thick, mutedred](0,0)--(3,0) node [midway, above] {$a$};
				\draw [thick, mutedblue](0,0)--(0,5) node [midway, right] {$b$};
				\draw [thick, mutedgreen](0,0)--(0, -4) node [midway, right] {$c$};

				\node [whitedot, label=right:$f_1$] at (0,4){};
				\node [whitedot, label=right:$f_2$] at (0,-4){};
				\node [whitedot, label=45:{$C$}] at (0,0){};

				\node [whitedot, label=-90:$v_1$] at (0,-5){};
				\node [whitedot, label=180:$v_2$] at (-3,0){};
				\node [whitedot, label=90:$v_3$] at (0,5){};
				\node [whitedot, label=0:$v_4$] at (3,0){};
			\end{axis}
		\end{tikzpicture}
		\subcaption{Ellisse verticale}
		\label{fig:ellisse verticale}
	\end{subfigure}
	\caption{Two subfigures side by side}
	\label{fig:ellisse verticale e orizzontale}
\end{figure}


Date le coordinate del centro $ c = \left(x_c, y_c\right) $, allora si può dimostrare facilmente che

\[
	\begin{aligned}
		\text{se } a^2 > b^2 & \to F_{1,2} = (x_C \pm c,\; y_C) \quad \text{dove } c = \sqrt{a^2 - b^2} \\
		\text{se } b^2 > a^2 & \to F_{1,2} = (x_C,\; y_C \pm c) \quad \text{dove } c = \sqrt{b^2 - a^2}
	\end{aligned}
\]
\subsubsection{Eccentricità}
L'ultimo concettto importante è quello di \textit{eccentricità}, che è una misura di quanto l'ellisse si discosta da essere una circonferenza. L'eccentricità $ e $ è definita come il rapporto tra la distanza dei fuochi $ c $ e la distanza dei vertici $ a $:
\[
	\begin{aligned}
		e & = \frac{c}{a} \quad \text{se } a^2 > b^2 \\
		e & = \frac{c}{b} \quad \text{se } b^2 > a^2
	\end{aligned}
\]
è importante notare i due casi limite per i valori dell'eccentricità:
\begin{itemize}
	\item Se $ e = 0 $, allora l'ellisse è una circonferenza, in quanto i fuochi coincidono con il centro ($ c=0 $)
	\item Se $ e = 1 $, allora l'ellisse degenera in un segmento in quanto i fuochi coincidono con i vertici ($ c=a $ o $ c=b $)
\end{itemize}

\subsection{Iperbole}
Iniziamo mostrando un esempio di iperbole:
\begin{figure}[H]
	\begin{tikzpicture}
		\begin{axis}[
				xmin=-6, xmax=6,
			...
			% \draw (0,-3)--(0,3);
			%
			\addplot[dashed] expression {x};
			\addplot[dashed] expression {-x};
			\addplot [thick] ({cosh(x)}, {sinh(x)});
			\addplot [thick] ({-cosh(x)}, {sinh(x)});

			\draw [thick, mutedred](0,0)--(1,0) node [midway, above] {$a$};
			\draw [thick, mutedblue](0,0)--(0,2) node [midway, right] {$b$};
			\draw [thick, mutedgreen](0,0)--(-2, 0) node [midway, above] {\contour{white}{$c$}};
			%
			% \node [whitedot, label=$f_1$] at (4,0){};
			% \node [whitedot, label=$f_2$] at (-4,0){};
			% \node [whitedot, label=45:{$C$}] at (0,0){};
			...
			\node [whitedot] at (0,2){};
			\node [whitedot, label=-90:$ C $] at (0,0){};

			\node [whitedot, label=$ f_1 $] at (2,0){};
			\node [whitedot, label=$ f_2 $] at (-2,0){};

			\node [whitedot, label=-90:$ v_2 $] at (-1,0){};
			\node [whitedot, label=-90:\contour{white}{$ v_2 $}] at (1,0){};
		\end{axis}
	\end{tikzpicture}
	\caption{Esempio di iperbole}
	\label{fig:iperbole}
\end{figure}
\begin{definizione}{Iperbole}
	L'iperbole è il luogo dei punti del piano per i quali la differenza delle distanze da due punti fissi, detti fuochi, è costante. In particolare, se $ P $ è un punto qualsiasi dell'iperbole e $ f_1 $ e $ f_2 $ sono i due fuochi, allora:
	\[
		\left|\overline{f_1P} - \overline{f_2P}\right| = \text{costante}
	\]
	più precisamente, se $ a $ è la distanza tra il centro dell'iperbole e uno dei vertici, e $ c $ è la distanza tra il centro dell'iperbole e uno dei fuochi, allora vale la relazione:
	\[
		\left|\overline{f_1P} - \overline{f_2P}\right| = 2a
	\]
\end{definizione}
Analizzando il caso in cui il semiasse trasverso è orizzontale, si ottiene la seguente formula:
\[
	\frac{\left(x - x_c\right)^2 }{a^2} - \frac{\left(y - y_c\right)^2 }{b^2} = 1
\]
se invece il semiasse trasverso è verticale, si ottiene la seguente formula:
\[
...
I vertici sono i punti indicati in \cref{fig:iperbole} con $ v_1 $ e $ v_2 $. Essi sono i punti di intersezione fra l'iperbole e l'asse trasverso. Date le coordinate del centro $ c = \left(x_c, y_c\right) $, allora è immediato vedere che questi hanno coordinate:
\begin{align*}
	v_1  = \left(x_c - a, y_c\right) \quad  v_2  = \left(x_c + a, y_c\right) &  & \text{se il semiasse trasverso è orizzontale} \\
	v_1  = \left(x_c, y_c - a\right) \quad  v_2  = \left(x_c, y_c + a\right) &  & \text{se il semiasse trasverso è verticale}
\end{align*}

\subsubsection{Fuochi}
A differenza dell'ellisse, i fuochi dell'iperbole non sono all'interno della curva, ma si trovano all'esterno. Essi sono i punti che definiscono l'iperbole, in quanto la differenza delle distanze da questi punti è costante. Questi punti sono simmetrici rispetto al centro dell'iperbole e sono posizionati sull'asse orizzontale se il semiasse trasverso è orizzontale, su quello verticale altrimenti. La semidistanza focale è data da:
\[
	c = \sqrt{a^2 + b^2}
\]

\subsubsection{Eccentricità}
Come nel caso dell'ellisse, anche per l'iperbole è possibile definire l'eccentricità, che è una misura di quanto l'iperbole si discosta da essere una parabola. L'eccentricità $ e $ è definita come il rapporto tra la distanza dei fuochi $ c $ e la distanza dei vertici $ a $:
\[
	e = \frac{c}{a}
\]
si noti tuttavia che in questo caso $ e $ assume valori maggiori di 1, in quanto $ c > a $ per definizione.
% In particolare, se $ e $ tende a 1, allora l'iperbole si avvicina sempre di più ad essere una parabola, mentre se $ e $ tende a infinito, allora l'iperbole si avvicina sempre di più ad essere un insieme di due rette parallele.

\subsubsection{Asintoti}
Il coefficiente angolare degli asintoti è dato da:
\[
	m = \pm \frac{b}{a}
\]

per trovare gli asintoti effettivi è duqnue sufficiente trovare le rette che passano per il centro dell'iperbole e che hanno coefficiente angolare $ m $ e $ -m $:
