class Solution {
  public int uniquePathsWithObstacles(int[][] obstacleGrid) {
    ...
    for (int i = 1; i < m; i++)
      obstacleGrid[i][0] = obstacleGrid[i][0] == 1 ? -1 : obstacleGrid[i - 1][0];

    for (int i = 1; i < n; i++)
      obstacleGrid[0][i] = obstacleGrid[0][i] == 1 ? -1 : obstacleGrid[0][i - 1];
  ...
  public static void main(String[] args) {
    ...
    (new Solution()).uniquePathsWithObstacles(mat);
  }
}
