...
\begin{table}[H]
	\begin{tabularx}{\linewidth}{lX}
		...
		\midrule
		\verb|s.length()|                     & Ritorna la lunghezza della stringa                         \\
		\verb|s.charAt(int index)|            & Ritorna il carattere in posizione \verb|index|             \\
		\verb|s.replace(char c1, char c2)|    & Sostituisce tutte le occorrenze di \verb|c1| con \verb|c2| \\
		\verb|s.equals(String str)|           & Verifica se due stringhe sono uguali                       \\
		\verb|s.equalsIgnoreCase(String str)| & Come sopra, ignorando maiuscole/minuscole                  \\
		\verb|s.compareTo(String str)|        & Confronto lessicografico (ordine alfabetico)               \\
		\verb|s.toCharArray()|                & Converte in un array di caratteri \verb|char[]|            \\
		\bottomrule
	\end{tabularx}
	\caption{Funzioni basi}
\end{table}
...
\begin{table}[H]
	\begin{tabularx}{\linewidth}{lX}
		...
		\verb|s.lastIndexOf(String str)|    & Ritorna l’indice dell’ultima occorrenza della sottostringa \verb|str| in \verb|s|     \\
		\verb|s.contains(CharSequence seq)| & Ritorna \verb|true| se la sequenza \verb|seq| è contenuta in \verb|s|                 \\
		\verb|s.startsWith(String prefix)|  & Verifica se \verb|s| inizia con la stringa indicata                                   \\
		\verb|s.endsWith(String suffix)|    & Verifica se \verb|s| termina con la stringa indicata                                  \\
		\bottomrule
	\end{tabularx}
	\caption{Funzioni di ricerca in stringhe}
\end{table}
...
\begin{table}[H]
	\begin{tabularx}{\linewidth}{lX}
		...
		\sfblue{Funzione}                      & \sfblue{Descrizione}                                                         \\
		\midrule
		\verb|s.substring(int beginIndex)|     & Restituisce la sottostringa da \verb|beginIndex| fino alla fine              \\
		\verb|s.substring(int begin, int end)| & Restituisce la sottostringa da \verb|begin| (incluso) a \verb|end| (escluso) \\
		\verb|s.split(String regex)|           & Divide la stringa in un array, usando l’espressione regolare come separatore \\
		\verb|String.join(delim, ...)|         & Unisce più stringhe usando il delimitatore specificato                       \\
		\verb|s.trim()|                        & Rimuove gli spazi (whitespace) iniziali e finali dalla stringa               \\
		\verb|s.strip()| (Java 11+)            & Rimuove spazi secondo le regole Unicode                                      \\
		\bottomrule
	\end{tabularx}
	\caption{Funzioni di splitting}
\end{table}
...
\begin{table}[H]
	\begin{tabularx}{\linewidth}{lX}
		...
		\sfblue{Funzione}      & \sfblue{Descrizione}                    \\
		\midrule
		\verb|s.replaceAll(String regex, |                               \\ \verb|String replacement)|   & Sostituisce tutte le occorrenze che corrispondono all’espressione regolare con un’altra stringa \\
		\verb|s.replaceFirst(String regex, |                             \\ \verb|String replacement)| & Sostituisce solo la prima occorrenza (secondo regex) \\
		\verb|s.toUpperCase()| & Converte tutti i caratteri in maiuscolo \\
		\verb|s.toLowerCase()| & Converte tutti i caratteri in minuscolo \\
		\bottomrule
	\end{tabularx}
	\caption{Funzioni di modifica}
\end{table}
...
\begin{esercizioL}{Plotting graphs}
	Scrivere del codice java per gestire un programma che crei il grafico approssimato di 3 tipi di funzioni:
	\begin{itemize}
		\item Polinomi
		\item Circonferenze
		\item Punti
	\end{itemize}
	Per ogni forma vi sarà un tipo di file diverso: \verb|poly.txt|, \verb|circ.txt| e \verb|points.txt|. Il formato dei file è il seguente:
	\vskip3mm
	\verb|poly.txt|: un polinomio per riga, con ignognita la lettera $ x $. Gli spazi vanno ignorati e gli esponenti possono apparire in qualsiasi ordine:
	\begin{lstlisting}[frame = none]
  2x^2 +x -1
  2x^2 -2x^3+x -1
  -1
  ...
  \end{lstlisting}
	...
	\verb|points.txt|: un punto definito da 2 coordinate
	\begin{lstlisting}[frame = none]
  (1, 2)
  (-1, 1)
  \end{lstlisting}

	\verb|circ.txt|: una circonferenza per riga, definita dal centro e dal raggio:
	\begin{lstlisting}[frame = none]
  (1, 2) 3
  (-1, 1) 6
  \end{lstlisting}
	I 3 file vanno letti e parsati correttamente. Successivamente si disegnino gli oggetti matematici in un piano cartesiano, salvando tutto in un file \verb|out.svg|. Il formato svg permette di esprimere grafice create con linee, come segue:
	\begin{lstlisting}[language = java, frame = none]
<svg width="20" height="10" xmlns="http://www.w3.org/2000/svg">
  <path d="M10 10 L 100 10 L 100 100 L 10 100 Z"
    fill="none"
    stroke="black"
    stroke-width="3" />
</svg>
  \end{lstlisting}
	In particolare, la sintassi per esprimere una linea spezzata è la seguente:
	\begin{itemize}
		\item \verb|M x y|: sposta la penna alla posizione \verb|(x, y)|
		\item \verb|L x y|: traccia una linea dalla posizione precedente a \verb|(x, y)|
		\item \verb|Z|: chiude il tracciato
	\end{itemize}
	nota che possono essere usati più tracciati, ad esempio:
	\begin{lstlisting}[language = java, frame = none]
<svg width="200" height="10" xmlns="http://www.w3.org/2000/svg">
  <path d="M10 10 L 100 10" fill="none" stroke="red" stroke-width="3" />
  <path d="M10 10 L 100 10" fill="none" stroke="green" stroke-width="3" />
</svg>
  \end{lstlisting}
...
\begin{esercizio}{Directory tree}
	Vi viene data una directory e dovete stampare su terminale una albero che rappresenti la directory, ad esempio, avendo le cartelle
	\begin{itemize}
		\item \verb|1|: contiene \verb|file1.txt| e \verb|file2.txt|
		\item \verb|2|: contiene \verb|file3.txt| e \verb|file4.txt|
		\item \verb|3|: contiene le cartelle \verb|1| e \verb|2|
	\end{itemize}
...
|- 2
   ...
   |- file4.txt
  \end{lstlisting}
	Dopo aver stampato l'albero, vanno stampate classifiche per ogni file trovato in ogni possibile sottodirectory, in particolare:
	\begin{itemize}
		\item Numero di file totali
		\item Tutte le diverse estensioni trovate, ciascuna con il relativo conteggio, ordinare in ordine decrescente
		\item Lunghezza media (in caratteri) di tutti i file trovati
	\end{itemize}
	Hint: si possono usare le funzioni seguenti per listare file e directories:
	\begin{lstlisting}[language = java, frame = none]
File dir = new File("path/to/your/directory");
File[] files = dir.listFiles(File::isFile);
File[] directories = dir.listFiles(File::isDirectory);
\end{lstlisting}
	Per concatenare diversi paths in maniera sicura, si può usare la funzione:
	\begin{lstlisting}[language = java, frame = none]
Paths.get("component1", "component2", ..., "componentn")
\end{lstlisting}


\end{esercizio}

\section{Consigli vari per la verifica}
\subsection{Consigli eclipse}
\begin{center}
	\begin{tabular}{l l l}
		\toprule
		\sfblue{Operazione}            & \sfblue{Menu}                              & \sfblue{Scorciatoia}    \\
		\midrule
		Go to definition (declaration) & Navigate $ \rightarrow  $ Open declaration & \verb|f3|               \\
		Format                         & Source $ \rightarrow  $ Format             & \verb|ctrl + shift + f| \\
		Rename                         & Refactor $ \rightarrow  $ Rename           & \verb|alt + ctrl + r|   \\
		\bottomrule
	\end{tabular}
\end{center}
...
\begin{itemize}
	\item Matcha le posizioni in cui splittare una stringa separata da virgole, gestendo spazi aggiuntivi:
	      \begin{center}
		      \verb|\s*,\s*|
	      \end{center}
	      ...
	      \begin{lstlisting}[frame = none]
Francesco   ,   Gallo   , 05/01/2000, 3452233445 , fr.ga@example.com
Chiara, Moretti, 17/09/1995, 3516677889, chiara.moretti@example.com
Alberto, Ferri, 21/06/1978, 3341234123, alberto.ferri@example.com 
          \end{lstlisting}
	\item Matcha in 3 gruppi diversi coefficiente, $ x $ e esponente di un polinomio, gestendo spazi aggiuntivi:
	      \begin{center}
		      \verb|([-\+]?\s*\d*?\s*)(x)((\s*\^\s*\d)?)|
	      \end{center}
	      testa su:
...
2 x
          \end{lstlisting}
	\item Matcha le mail in formato \verb|nome.cognome@dominio.com| dove dominio può essere \verb|example| o \verb|gmail|:
	      \begin{center}
		      \verb|\w+\.\w+@(example|\texttt{|}\verb|gmail)\.com|
	      \end{center}
	      testa su:

	      \begin{lstlisting}[frame = none]
Francesco, Gallo, 05/01/2000, 3452233445 , fra.gallo@example.com
Chiara, Moretti, 17/09/1995, 3516677889, chiara.moretti@example.com
Alberto, Ferri, 21/06/1978, 3341234123, alberto.ferri@example.com 
          \end{lstlisting}
\end{itemize}
...
\footnotesize
\sfblue{Files-lettura}
\begin{lstlisting}[language = java, frame = none, basicstyle=\ttfamily\footnotesize]
// Leggo parola/intero/float uno per volta
Scanner scanner = new Scanner(new File("path/to/file.txt")));

// Leggo intero contenuto linea per linea in lista di stringhe
...
\sfblue{Files-scrittura}
\begin{lstlisting}[language = java, frame = none, basicstyle=\ttfamily\footnotesize]
// Scrivo stringa e chiudo file immediatamente
Files.writeString(Paths.get("file.txt"), "contenuto", <StandardOpenOption>);

StandardOpenOption.TRUNCATE_EXISTING // se file esiste cancella contenuto
StandardOpenOption.APPEND // se file esite scrive in fondo

// Scrivo lista di stringhe
String array[] = { "line1", "line2" }; // va convertito con Arrays.asList()
ArrayList<String> arrayList = new ArrayList<>(Arrays.asList("line1", "line2"));

Files.write(Paths.get("file.txt"), Arrays.asList(array), <StandardOpenOption>);
Files.write(Paths.get("file.txt"), arrayList, <StandardOpenOption>);
\end{lstlisting}

\sfblue{Features Eclipse}
\begin{center}
	\begin{tabular}{l l l}
		\toprule
		\sfblue{Operazione}            & \sfblue{Menu}                                             & \sfblue{Scorciatoia}    \\
		...
		Go to definition (declaration) & Navigate $ \rightarrow  $ Open declaration                & \verb|f3|               \\
		Format                         & Source $ \rightarrow  $ Format                            & \verb|ctrl + shift + f| \\
		Automatically import libraries & Source $ \rightarrow  $ Organize imports                  & \verb|ctrl + shift + o| \\
		Rename                         & Refactor $ \rightarrow  $ Rename                          & \verb|alt + ctrl + r|   \\
		Generate getters/setters       & Source $ \rightarrow  $ Generate getters/setters          & \verb|nessuna|          \\
		Generate toString              & Source $ \rightarrow  $ Generate toString                 & \verb|nessuna|          \\
		Generate constructor           & Source $ \rightarrow  $ Generate constructor using fields & \verb|nessuna|          \\
		\bottomrule
	\end{tabular}
\end{center}
\sfblue{Date}
...
\end{lstlisting}
l'oggetto \verb|(o1, o2) -> <criterio ordering>| serve per definire un ordinamento fra due istanze di \verb|SomeClass| (\verb|o1|, \verb|o2|). \verb|<criterio ordinamento>| deve essere un intero $ < 0 $ se $ o_1 < o_1 $, $ = 0 $ se $ o_1 = o_2 $ e $ > 0 $ se $ o_1 > o_2 $. Ad es, per ordinare rispetto al campo \verb|x| di una classe: \verb|list.sort((o1, o2) -> o1.x - o2.x)|

\vskip3mm
\sfblue{Enum con valori associati}

\begin{lstlisting}[language = java, frame = none, basicstyle=\ttfamily\footnotesize]
enum Direction {
  UP("Su", 1), DOWN("Giù", 3), LEFT("Sinistra", 2), RIGHT("Destra", 4)
  private final String ita; private final int val;
  private Direction(String ita, int val) {
    this.ita = ita; this.val = val;
  }
}
\end{lstlisting}

\vskip3mm
\sfblue{Funzioni stringhe}
\begin{lstlisting}[language = java, frame = none, basicstyle=\ttfamily\footnotesize]
// sostituzione
s.replace(char c1, char c2)
...
s.toUpperCase()
s.toLowerCase()
s.indexOf(String stringa)
  \end{lstlisting}
\end{minipage}

\vskip3mm
\sfblue{Espressioni regolari}
\vskip3mm
\begin{center}
	\begin{tabularx}{0.49\textwidth}{l X}
		\toprule
		\multicolumn{2}{c}{\sfblue{Wildcards}}   \\
		\midrule
		{\lstinline!.!}   & Qualsiasi carattere  \\
		{\lstinline!\\s!} & Whitespace character \\
		{\lstinline!\\d!} & Digit, cifra         \\
		{\lstinline!\\w!} & Word character       \\
		\bottomrule
	\end{tabularx}
	\hfill
	\begin{tabularx}{0.49\textwidth}{l X}
		\toprule
		\multicolumn{2}{c}{\sfblue{Quantificatori}}        \\
		\midrule
		{\lstinline!+!}     & 1 o più                      \\
		{\lstinline!*!}     & 0 o più                      \\
		{\lstinline!?!}     & 0 o 1                        \\
		{\lstinline!\{n\}!} & esattamente $ n $ occorrenze \\
		\bottomrule
	\end{tabularx}
\end{center}
%
...
\begin{center}
	\begin{tabularx}{\textwidth}{l X}
		\toprule
		\sfblue{Regex}                                            & \sfblue{Descrizione}                        \\
		\midrule
		{\lstinline!\\w+\\.\\w+@(domain1|domain2|domainx)\\.com!} & matcha "qualcosa.qualcosa@dominio.com"      \\
		{\lstinline!\\s*,\\s*!}                                   & matcha una virgola con spazi a sx e dx      \\
		{\lstinline!\\d\{2\}/\\d\{2\}/\\d\{4\}!}                  & matcha una data nel formato "gg/mm/aaaa"    \\
		{\lstinline!https?://[\\w-]+!}                            & matcha un URL HTTP o HTTPS                  \\
		{\lstinline!([A-Z][a-z]+)!}                               & matcha parole che iniziano con la maiuscola \\
		\bottomrule
	\end{tabularx}
\end{center}
\begin{lstlisting}[language = java, frame = none]
...
\sfblue{Altro}
\begin{lstlisting}[language = java, frame = none]
// Ottenere path directory corrente
